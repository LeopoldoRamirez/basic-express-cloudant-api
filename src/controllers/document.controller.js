...
function generateId( partitionId ){
    const key = uuid4();
    return  [ partitionId, key ].join(":");
}
...
const getDocumentById = async(request, response)=>{
    ...
    try {
        const connection =getConnection();
        const document =  await connection.get( id );

        return buildResponse( response, 200, { msg: "get document by id" , document });
    } catch (error) {
        console.error(error);
        return buildResponse( response, 500, {} ); ;    
    }
...
const updateDocument = async(request, response)=>{
    ...
    try {
        ...
        const document ={ ... revision,... sentData };
...
const saveDocument = async( request, response )=>{
    ...
    noteData['_id'] =  generateId(partitionId ); 
...
const saveDocsByPartition = async(request, response )=>{
    ...
    //generate the id
    docs.forEach( doc => {        
        doc._id =  generateId(partitionId );
    });
